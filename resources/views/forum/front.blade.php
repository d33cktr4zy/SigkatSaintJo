...
@foreach($kat->all() as $Kategori)
				<tr class="kategori" style="">
					<td colspan="4">{!! $Kategori->nm_kategori !!}</td>
				</tr>
				@foreach($Kategori->topiks as $topik)
				<tr>
					<td class="topik"><span class="topikTitle">{!! $topik->nm_topik !!}</span><br/>
						<span class="topikDesc">
							{!! str_limit($topik->desc_topic,100,'...') !!}
						</span>

					</td>
					<td class="topikCount">{!! \stjo\Model\User::where('id', $topik->id_pembuat)->pluck('username') !!}</td>
